...
- [~] **Tinh chỉnh mascot/icon v2 (đã LÊN PRODUCTION 06/08/2026)** — người dùng lệnh xóa hẳn v1 ("rác"), v2 thay vào `frontend/public/brand/{mascot,icons}` dù "chưa ổn hẳn" — sửa tiếp trên nền v2: mascot 8 file (4 pose × 2 nền), 42 icon, brand:check đã nới theo chuẩn v2 (mascot viewBox 220×252 + whitelist amber/mint, hex trần cho gradient). Bộ lockup dọc ĐÃ XÓA (file + code sinh + test) theo lệnh người dùng. Nguồn + hàm sinh: `frontend/scripts/brand/proto-v2/`. Còn lại: tinh chỉnh thẩm mỹ tiếp theo cảm nhận người dùng, cập nhật ảnh PNG export nếu cần (CẢ HAI)
