...
- [~] **Tinh chỉnh icon v2** (42 file đã lên production 06/08/2026, người dùng duyệt "tạm ok") — các điểm em ghi nhận cần soi: `incorrect` bản filled mất hình chữ ✗, `spark` khác ẩn dụ giữa outline và filled, `learner-profile` filled bị cắt cằm, `learning-evidence` filled huy hiệu tràn mép giấy. Cũng cần xuất lại PNG nếu bộ icon đổi (CẢ HAI)
