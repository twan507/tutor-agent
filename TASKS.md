...
- [~] **Tinh chỉnh mascot/icon v2 (đã LÊN PRODUCTION 06/08/2026)** — người dùng lệnh xóa hẳn v1 ("rác"), v2 thay vào `frontend/public/brand/{mascot,icons}` dù "chưa ổn hẳn" — sửa tiếp trên nền v2: mascot 6 file (3 pose × 2 nền), 42 icon, brand:check đã nới theo chuẩn v2 (mascot viewBox 220×252 + whitelist amber/mint, hex trần cho gradient). ĐÃ XÓA theo lệnh người dùng: bộ lockup dọc (06/08); nhánh motif đường bay chấm gạch + pattern nền + pose `flying` + toàn bộ animation (07/08) — xóa cả file lẫn code sinh lẫn test. Nguồn + hàm sinh: `frontend/scripts/brand/proto-v2/`. Còn lại: tinh chỉnh thẩm mỹ tiếp theo cảm nhận người dùng, cập nhật ảnh PNG export nếu cần (CẢ HAI)
