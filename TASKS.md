...
- [~] **Hoàn thiện mascot + icon v2 rồi tích hợp production** — vòng mockup 06/08/2026 xong, người dùng duyệt "tạm ổn, chưa ổn hẳn": mascot v2 tỷ lệ chuẩn hóa từ SVG trace (đầu ≈ 1.05× bề ngang thân, cánh giọt nước ngang, chi tiết tại research doc), icon v2 "tạm ok". Prototype + mọi hàm sinh SVG: `frontend/scripts/brand/proto-v2/` (chạy `node assemble.mjs`). Còn lại: tinh chỉnh cuối theo cảm nhận người dùng → spec tích hợp (thay mascot/icon production, nới rule brand:check cho gradient, cập nhật sổ tay §3.3-3.4). Chi tiết mục "KẾT QUẢ vòng mockup v2" trong `docs/research/danh-gia-tham-my-mascot-icon-2026-08-05.md` (CẢ HAI)
