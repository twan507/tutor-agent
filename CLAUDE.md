...
2. **Kỷ luật git**: khi dự án đã có code chạy, làm feature/fix trên nhánh riêng (`feat/...`, `fix/...`), không commit thẳng main. Commit theo Conventional Commits (`feat:`, `fix:`, `docs:`, `chore:`, `test:`...), commit nhỏ một mục đích. **Commit theo mốc tiến trình**: mỗi mốc hoàn thành (một quyết định được chốt, một việc xong) = một commit riêng ngay lúc đó — không dồn nhiều mốc vào một commit, để diff truy ngược được từng quyết định. Push sau khi commit. **Công khai nhánh đang làm việc**: ngay khi tạo nhánh mới hoặc chuyển sang nhánh khác, báo cho người dùng tên nhánh + đường dẫn thư mục làm việc (worktree nào), và push sớm để nhánh hiện trên remote — người dùng phải tự kiểm tra được đang làm ở đâu, không phải hỏi mới biết. Cũng áp dụng cho nhánh/worktree do công cụ tự tạo, không chỉ nhánh agent tự lập.
3. **Không bypass cơ chế kiểm soát**: cấm `--no-verify`, force push, `rm -rf`, sửa `.env*` / `.claude/settings.json` / `.claude/hooks/` — agent gõ trực tiếp các lệnh này đã bị hook trong `.claude/settings.json` chặn cứng; gặp chặn thì báo người dùng, không tìm đường vòng. Riêng `docker volume rm/prune` là ngoại lệ có kiểm soát: hook chặn agent gõ trực tiếp, nhưng lệnh `pnpm docker-clean` (chạy `docker volume prune` bên trong `scripts/stack.mjs`) là lệnh hợp lệ, được cho phép — có hàng rào riêng bên trong script (kiểm tra phiên bản Docker Engine trước khi prune, kiểm tra bất biến volume `tutor-infra_pgdata` trước/sau).
...
- Proxy: **Nginx**; hạ tầng: **VPS + Docker Compose 2 tầng** — `infra` (postgres, CI không bao giờ đụng) + `app`; **5 lệnh gõ qua pnpm**: `pnpm dev-start` / `pnpm dev-stop` / `pnpm docker-up` / `pnpm docker-down` / `pnpm docker-clean` (khai báo trong `package.json` gốc, logic trong `scripts/stack.mjs`; wrapper `.bat`/`.sh` cũ lưu trữ trong `scripts/` — xem README.md mục "Chạy dự án"). Deploy VPS **KHÔNG** dùng script này — CI/CD giai đoạn 2 gọi thẳng `docker compose` qua SSH, VPS chỉ cần Docker, không cần Node.
...
- **Mascot**: đom đóm/đốm sáng, sáng dần theo tiến bộ. Mô hình cảm xúc **Forest, không Duolingo**: thưởng cộng dồn, nghỉ = mờ trung tính; CẤM mắt to nhìn chằm chằm (gợi giám sát), CẤM biểu cảm buồn/thất vọng khi sai/nghỉ (trừng phạt cảm xúc). Hai dạng, hai địa bàn (chốt 05/08/2026): đốm sáng tối giản = wordmark (chấm chữ i) + app icon — dạng duy nhất trong logo; đom đóm đầy đủ geometric (không baby-face) = chỉ minh họa (hero, empty state, social). **CẤM nét đứt trang trí** (motif "đường bay chấm gạch" đã loại 07/08/2026 cùng pattern nền, pose `flying` và toàn bộ animation — sổ tay thương hiệu §3.3); ngoại lệ duy nhất là icon `learning-path`, nơi nét đứt là hình vẽ của lộ trình chứ không phải họa tiết.
