...
> **RÀO CHO DỰ ÁN NÀY (tutor-agent/Rangi) — đọc trước khi dùng.** Brand identity đã CHỐT (CLAUDE.md mục "Bộ nhận diện thương hiệu": màu amber/emerald + semantic token, font PhuDu/Be Vietnam Pro/Inter/Lexend). Vì vậy trong repo này **CẤM** dùng `--design-system` và các domain `color`, `typography`, `google-fonts` — skill sẽ đề xuất palette/font đè lên brand đã khóa. Chỉ dùng các domain không đụng màu/font: `ux`, `style` (bỏ qua cột Primary Colors), `product`, `landing`, `chart`, `icons`, `gsap`/`motion`, `react`, `web` và `--stack <name>`. Màu và font luôn lấy từ semantic token của dự án.
